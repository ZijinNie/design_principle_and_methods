...
public class Odometer implements Runnable {
  ...
  public void run() {
    ...
    //store the reseted value as the last tacho count
    lastleftMotorTachoCount = leftMotor.getTachoCount(); 
    lastleftMotorTachoCount = rightMotor.getTachoCount();
    
    
    double distL,  //distance that the left wheel has passed in the last cycle
           distR,  //distance that the right wheel has passed in the last cycle
           deltaD, //distance difference of left wheel and right wheel
           deltaT, //change in position angel 
    ...
    while (true) {
      ...
      leftMotorTachoCount = leftMotor.getTachoCount();  //get current left tacho coun
      rightMotorTachoCount = rightMotor.getTachoCount();    //get current right whell tacho count

      // Calculate new robot position based on tachometer counts
      
      // Calculate left wheel and right wheel distance
